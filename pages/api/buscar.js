async function buscar(request, response) {
  ...
  async function Magia(autocomplete){
    let listaDePrdutos = []
    let result = await fetch(`https://b2c-sm-www-api-production4.sitemercado.com.br/api/v1/b2c/5553/product/load_search/${autocomplete}`, {"credentials": "omit",
    "headers": {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:85.0) Gecko/20100101 Firefox/85.0",
        "Accept": "application/json, text/plain, */*",
        "Accept-Language": "pt-BR,pt;q=0.8,en-US;q=0.5,en;q=0.3",
        "sm-mmc": "2020.03.22-0",
        "sm-token": "{\"Location\":{\"Latitude\":-27.596019744873,\"Longitude\":-48.5562591552734,\"Country\":null,\"CountryCode\":null,\"IP\":null},\"IdClientAddress\":0,\"IsDelivery\":true,\"IdLoja\":5553,\"IdRede\":3107,\"DateBuild\":\"2021-01-21T21:47:19.0647343\",\"Version\":\"2019.07.24-0\",\"CellPhoneModel\":null,\"AndroidVersion\":null,\"DeviceUUID\":null}",
        "Pragma": "no-cache",
        "Cache-Control": "no-cache"
    },
    "referrer": "https://www.sitemercado.com.br/supermercadosmagia/florianopolis-loja-canasvieiras-canasvieiras-avenida-das-nacoes/busca/arroz",
    "method": "GET",
    "mode": "cors"})
    let produtosJson = await result.json()
  ...
  async function Angeloni(nomeAPesquisar){
    let listaDePrdutos = []
    let result = await fetch(`https://www.angeloni.com.br/super/autosuggest.json/browse?Dy=1&collection=/content/Web/super/AutoSuggest&Ntt=${nomeAPesquisar}*`)
    let produtosJson = await result.json()
    if (produtosJson.autoSuggest[0].dimensionSearchGroups[1] == undefined){
      console.log('Não foi encontrado nada')
    }else{
    for (var i = 0; i< produtosJson.autoSuggest[0].dimensionSearchGroups[1].dimensionSearchValues.length;i++){
      let prop = produtosJson.autoSuggest[0].dimensionSearchGroups[1].dimensionSearchValues[i].properties.split(',')
      let price = prop[3].replace(' skuListPrice=R$ ','')
      let img = prop[14].replace(' skuThumbnailUrl=//','http://')
      listaDePrdutos.push({
        nome: produtosJson.autoSuggest[0].dimensionSearchGroups[1].dimensionSearchValues[i].label,
  ...
  async function Bistek(nomeAPesquisar){
    let listaDePrdutos = []
    let result = await fetch(`https://www.bistek.com.br/searchautocomplete/ajax/suggest/?q=${nomeAPesquisar}&store_id=2&cat=false&_=1611284495412`)
    let produtosJson = await result.json()
    for(var i = 0; i < produtosJson.indices[0].items.length ; i++){
      let aux =  produtosJson.indices[0].items[i].price.split('data-price-amount=\"')
      let aux2 = aux[1].split('"')
      listaDePrdutos.push({
        nome: produtosJson.indices[0].items[i].name,
        preco: parseFloat(aux2[0]),
        urlImg: produtosJson.indices[0].items[i].image
      });
    }
